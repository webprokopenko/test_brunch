<?php
class TechnologyBuyController
{
    public function actionIndex($page=1)
    {
        $page = intval($page);
        $technology_all_list = Technology::getAllTechnology($page,1);


        // Общее количетсво товаров (необходимо для постраничной навигации)
        $total = Technology::getTotalTechnology(1);

        // Создаем объект Pagination - постраничная навигация
        $pagination = new Pagination($total, $page, Technology::SHOW_BY_DEFAULT, 'page-');

        //Подключаем вид
        require_once(ROOT.'/views/technology/technologybuy.php');
