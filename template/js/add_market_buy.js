    ...
    //Функция добавления объявления о продаже
        _market_buy_add = function(){
            ...
            defObject.done(function(ans){
                if(ans.error==0){
                    $(".popup__msg-buy").text("Объявление успешно добавлено").removeClass('error').addClass("sucess");
                    window.location.href = "/marketbuy/";
                }
                else{
        ...
        _ajaxForm =   function (form,url){
            var data = form.serialize(),
                defObj = $.ajax({
                    ...
                    $(".popup__msg-buy").text(ans).removeClass('sucess').addClass("error");
                });
            return defObj;
        },
